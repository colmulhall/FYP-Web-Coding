	...
	if(!empty($html))    //if any html is actually returned
	{
		$park_doc->loadHTML($html);
 	     ...
 	     $xpath = new DOMXPath($park_doc);  //DOMXPath allows queries with the DOM document. 
 	 	 ...
 	 	 $event_title = $xpath->query('//p');  //gets the event title, ignores any other h1 headings on the page
 		 $event_desc = $xpath->query('//p');  //gets the event description
		...
		if($event_title->length > 0)
		{
			...
			$full_event_title = '';
			$i = 0;
	        foreach($event_title as $node) 
	        {
	            $i++;
	            if($i == 2)
	            {
	            	$full_event_title .= $node->textContent .= " ";

		            //replace unreadable characters from the description (e.g. &nbsp;)
		            $full_event_title = utf8_decode($full_event_title);
		            $temp = utf8_decode('Â');
		            $full_event_title = str_replace($temp, '', $full_event_title);
		            $full_event_title = str_replace("?", " ", $full_event_title);
		        }
	        }
